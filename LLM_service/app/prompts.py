...
14. Если в вопросе, вариантах ответа или объяснении нужны математические формулы, записывай их в LaTeX: inline через \\(...\\), отдельные формулы через \\[...\\]. Не используй plain text вместо формул, если LaTeX делает запись точнее.
""".strip()
...
def build_user_prompt(request: GenerationRequest) -> str:
    ...
    parts = [
        ...
        "Если нужны формулы, используй LaTeX-разметку: \\(...\\) для inline-формул и \\[...\\] для отдельных формул.",
    ]
...
- Математические формулы в question, options.text и explanation записывай в LaTeX через \\(...\\) или \\[...\\].
""".strip()
    )
