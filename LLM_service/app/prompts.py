...
13. Не добавляй игровые механики, ставки, x2, кота в мешке, баллы или номиналы, кроме обязательного поля nominal для вопросов 100k1.
14. Если в вопросе, вариантах ответа или объяснении нужны математические формулы, записывай их только в LaTeX delimiters: inline через \\(...\\), отдельные формулы через \\[...\\]. Не пиши сырой TeX или формулы без delimiters: вместо A=B пиши \\(A=B\\), вместо a_{ij}=b_{ij} пиши \\(a_{ij}=b_{ij}\\).
""".strip()
...
def build_user_prompt(request: GenerationRequest) -> str:
    ...
    parts = [
        ...
        type_rules,
        "Строго генерируй только перечисленные выше типы. Если указан один тип, все вопросы должны быть именно этого типа. Не заменяй 100k1 на single_choice, multiple_choice или true_false.",
        ...
        "Если нужны формулы, используй LaTeX-разметку: \\(...\\) для inline-формул и \\[...\\] для отдельных формул. Любая формула должна быть внутри delimiters: пиши \\(A=B\\), \\(a_{ij}=b_{ij}\\), \\(i,j\\), а не A=B, a_{ij}=b_{ij}, i, j.",
    ]
...
- Значение поля type у каждого вопроса должно быть только из перечисленных в запросе типов.
- Если в запросе есть только 100k1, у каждого вопроса type должен быть "100k1"; не используй "single_choice", "multiple_choice" или "true_false".
...
- Математические формулы в question, options.text и explanation записывай в LaTeX через \\(...\\) или \\[...\\]. Не оставляй формулы без delimiters: неверно `A=B` или `a_{ij}=b_{ij}`, верно `\\(A=B\\)` и `\\(a_{ij}=b_{ij}\\)`.
""".strip()
    )
